/*
  # Add song plays earnings system

  1. New Tables
    - `song_plays` - Tracks individual song plays
    - `play_earnings` - Records earnings from plays
    - `play_rates` - Configures earnings per play

  2. Changes
    - Add play count tracking
    - Add earnings calculation functions
    - Add automatic earnings processing
...
-- Create indexes
CREATE INDEX idx_song_plays_song ON song_plays(song_id);
CREATE INDEX idx_song_plays_artist ON song_plays(artist_id);
CREATE INDEX idx_song_plays_date ON song_plays(played_at);
CREATE INDEX idx_play_earnings_artist ON play_earnings(artist_id);
CREATE INDEX idx_play_earnings_period ON play_earnings(period_start, period_end);
