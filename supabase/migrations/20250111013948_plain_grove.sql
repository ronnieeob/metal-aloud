/*
  # Update Commission Structure and Functions

  1. Changes
    - Implements tiered commission rates based on sales volume
    - Adds commission tracking and reporting
    - Creates functions for commission calculations
    - Adds commission history tracking

  2. Commission Structure
    - Base rate: 8% (standard industry rate)
    - Volume discounts:
      - $0-$1,000: 8%
      - $1,001-$5,000: 7%
      - $5,001-$10,000: 6%
...
DROP FUNCTION IF EXISTS calculate_artist_earnings(decimal);
...
-- Create function to calculate earnings with new return type
...
-- Create indexes
CREATE INDEX idx_commission_history_artist ON commission_history(artist_id);
CREATE INDEX idx_commission_history_period ON commission_history(period_start, period_end);
CREATE INDEX idx_commission_history_status ON commission_history(payout_status);