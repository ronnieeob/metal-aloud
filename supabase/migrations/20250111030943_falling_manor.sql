/*
  # Commission Tiers and Earnings Tracking

  1. New Tables
    - commission_tiers: Stores tiered commission rates
    - commission_history: Tracks artist earnings and commissions
  
  2. Functions
    - calculate_commission_rate: Determines rate based on amount
    - calculate_artist_earnings: Calculates net earnings and commission
  
  3. Views
    - artist_earnings_summary: Summarizes earnings by artist and period
*/
...
DROP INDEX IF EXISTS idx_commission_history_artist;
DROP INDEX IF EXISTS idx_commission_history_period;
DROP INDEX IF EXISTS idx_commission_history_status;
...
CREATE POLICY "Commission tiers are viewable by all"
  ON commission_tiers FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "Artists can view own commission history"
  ON commission_history FOR SELECT
  TO authenticated
  USING (artist_id = auth.uid());

-- Create new indexes with unique names
CREATE INDEX IF NOT EXISTS commission_history_artist_idx 
  ON commission_history(artist_id);
CREATE INDEX IF NOT EXISTS commission_history_period_idx 
  ON commission_history(period_start, period_end);
CREATE INDEX IF NOT EXISTS commission_history_status_idx 
