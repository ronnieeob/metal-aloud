...
CREATE POLICY "View own settings policy v3"
  ON user_settings FOR SELECT
  TO authenticated
  USING (user_id = auth.uid());

CREATE POLICY "Update own settings policy v3"
  ON user_settings FOR UPDATE
  TO authenticated
  USING (user_id = auth.uid())
  WITH CHECK (user_id = auth.uid());

CREATE POLICY "Insert own settings policy v3"
  ON user_settings FOR INSERT
  TO authenticated
  WITH CHECK (user_id = auth.uid());

-- Create index if not exists
CREATE INDEX IF NOT EXISTS idx_user_settings_user_v3 ON user_settings(user_id);
